...
export default function MixAndMatch({ gender, onOpenSavedLooks, onOpenMatchAnalysis }: Props) {
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto pt-20 pb-40 bg-surface min-h-screen relative">
      ...
      <section className="px-6 mb-4 mt-2">
        <motion.div
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h3 className="font-serif italic text-2xl group-hover:text-primary transition-colors">Saved Looks 가기</h3>
      ...
      {/* NEW: Primary Action to move to next page */}
      <section className="px-6 mb-8">
        <button
          onClick={onOpenMatchAnalysis}
          className="w-full bg-primary text-white py-5 rounded-2xl font-bold text-sm uppercase tracking-[0.2em] shadow-xl hover:bg-primary/90 transition-all flex items-center justify-center gap-3 group"
        >
          나의 전체 옷장 AI 매칭 분석 시작 ➔
        ...
        {selectedItems.length > 0 && !analysisDone && (
          ...
          >
            <button
              onClick={() => {
                handleAnalysis();
                setTimeout(() => onOpenMatchAnalysis?.(), 2000);
              }}
