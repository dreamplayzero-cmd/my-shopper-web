...
export default function App() {
  ...
  const [psychologyState, setPsychologyState] = useState('CALM');

  useEffect(() => {
    const root = document.documentElement;
    root.style.transition = 'all 0.5s ease-in-out';

    if (mbti === 'INFJ' && psychologyState === 'CALM') {
      root.style.setProperty('--app-primary', '#004d40');
      root.style.setProperty('--app-secondary-container', '#f1f8e9');
      root.style.setProperty('--app-highlight-bg', 'linear-gradient(135deg, #004d40 0%, #6d4c41 100%)');
    } else if (mbti === 'ENFP' && psychologyState === 'ENERGY') {
      root.style.setProperty('--app-primary', '#ff3d00');
      root.style.setProperty('--app-secondary-container', '#fffde7');
      root.style.setProperty('--app-highlight-bg', 'linear-gradient(135deg, #ff3d00 0%, #ffeb3b 100%)');
    } else if (mbti === 'INTJ' && psychologyState === 'CONFIDENCE') {
      root.style.setProperty('--app-primary', '#212121');
      root.style.setProperty('--app-secondary-container', '#f3e5f5');
      root.style.setProperty('--app-highlight-bg', 'linear-gradient(135deg, #212121 0%, #6a1b9a 100%)');
    } else if (mbti === 'ENTP' && psychologyState === 'ENERGY') {
      root.style.setProperty('--app-primary', '#d50000');
      root.style.setProperty('--app-secondary-container', '#eceff1');
      root.style.setProperty('--app-highlight-bg', 'linear-gradient(135deg, #d50000 0%, #1a237e 100%)');
    } else {
      root.style.setProperty('--app-primary', '#000000');
      root.style.setProperty('--app-secondary-container', '#e5e2dd');
      root.style.setProperty('--app-highlight-bg', 'transparent');
    }
  }, [mbti, psychologyState]);

  useEffect(() => {
    // Persistence disabled for demo flow to ensure step-by-step visibility
    /*
  ...
  return (
    <div className="h-full w-full overflow-y-auto bg-surface selection:bg-secondary-container safe-area-padding">
        ...
        >
          {currentScreen === 'closet' && (
            <MixAndMatch
              ...
              onOpenSavedLooks={() => setCurrentScreen('saved')}
              onOpenMatchAnalysis={() => setCurrentScreen('match')}
            />
          )}
          {currentScreen === 'saved' && <SavedLooks gender={selectedGender!} onBack={() => setCurrentScreen('closet')} />}
          {currentScreen === 'recommend' && <Suggestions onResetDNA={handleResetDNA} gender={selectedGender!} selectedPersonas={selectedPersonas} onGoToWardrobe={() => setCurrentScreen('closet')} onOpenMatchAnalysis={() => setCurrentScreen('match')} onOpenProfile={() => setCurrentScreen('profile')} />}
      ...
      {/* Theme Highlight Layer */}
      <div className="theme-highlight-layer" />
