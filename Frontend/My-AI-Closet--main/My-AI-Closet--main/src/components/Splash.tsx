...
export default function Splash({ onLogin }: { onLogin: (type: string) => void }) {
  return (
    ...
    >
      {/* Editorial Overlay for better readability */}
      ...
      <section className="w-full max-w-[400px] px-6 flex flex-col gap-8 z-10">
        ...
        <div className="flex flex-col gap-3">
          <motion.button
            ...
            className="flex items-center justify-center w-full h-14 bg-[#FEE500] rounded-full text-sm font-semibold transition-colors shadow-md"
          >
            카카오 계정으로 로그인
          </motion.button>
          ...
          <motion.button
            ...
            className="flex items-center justify-center w-full h-14 bg-white border border-outline-variant rounded-full text-sm font-semibold hover:bg-surface-container-low transition-colors shadow-sm"
          >
            구글 계정으로 계속하기
          </motion.button>
        </div>
