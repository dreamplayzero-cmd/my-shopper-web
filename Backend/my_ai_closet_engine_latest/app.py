...
from core.recommender import recommend_outfit, scan_missing_essentials, recommend_with_sqlite

# 페이지 기본 설정
st.set_page_config(page_title="My Shopper", page_icon="👗", layout="wide")

# 사이드바에서 유저 프로필 입력 (심리 상태 추가)
with st.sidebar:
    st.header("👤 내 스타일 프로필")
    user_mbti = st.selectbox("MBTI", ["INFJ", "ENFP", "INTJ", "ENTP", "ISTJ", "ESTJ", "Unknown"], index=0)
    user_state = st.radio("심리 상태 (Psychology State)", ["CALM", "ENERGY", "CONFIDENCE"], index=0)
    st.session_state["user_profile"] = {"mbti": user_mbti, "state": user_state}
