...
# ========================================================
# 4. [Grok's Aggressive CV Layer] 고속 의류 디텍션 및 Mock 매핑
# ========================================================
import time
import random

def detect_clothing_cv_light(image_path):
    """
    YOLO/OpenCV 백엔드를 모사한 0.1초 초고속 카테고리 디텍터.
    실제 원단 분석 리스크를 배제하고 카테고리 기반 Mock 데이터 매핑을 수행함.
    """
    ...
    # 파일명 기반 힌트 추출 (분석 성공률 제고)
    fn = os.path.basename(image_path).lower()
    
    # 디텍션 카테고리 정의
    categories = ["반팔 티셔츠", "린넨 셔츠", "가벼운 자켓", "여름용 숏팬츠"]
    
    # 매핑 로직
    detected = "반팔 티셔츠"
    if any(k in fn for k in ["shirt", "셔츠"]):
        detected = "린넨 셔츠"
    elif any(k in fn for k in ["jacket", "자켓", "blazer"]):
        detected = "가벼운 자켓"
    elif any(k in fn for k in ["pants", "shorts", "바지", "팬츠"]):
        detected = "여름용 숏팬츠"
        
    # [여름철 기능성 소재 메타 태그] 매핑 데이터
    material_meta = {
        "반팔 티셔츠": {"material": "기능성 쿨링 원단", "feature": "흡습속건, 체온 저하 효과"},
        "린넨 셔츠": {"material": "천연 린넨 소재", "feature": "통기성 극대화, 내추럴 텍스처"},
        "가벼운 자켓": {"material": "시어서커 지지미", "feature": "피부 접촉 최소화, 쾌적함 유지"},
        "여름용 숏팬츠": {"material": "나일론 타슬란", "feature": "생활 방수 및 고탄성 활동성"}
    }
    
    meta = material_meta.get(detected, {"material": "범용 여름 소재", "feature": "기본 통기성"})
...
def get_gemini_color_psychology_prompt(user_mbti, weather_data, color_tone="웜톤"):
    """
    색채심리 + LLM 프롬프트 고속 파이프라인 전용 프롬프트 생성
    MBTI와 날씨 데이터를 결합한 초개인화 서사 구조 설계
    """
    prompt = f"""
    [패션 에디터 & 색채심리학자 페르소나]
    - 사용자 MBTI: {user_mbti}
    - 현재 날씨: {weather_data}
    - 분석된 퍼스널 컬러: {color_tone}

    위 데이터를 기반으로 다음의 서사를 작성해줘:
    1. 오늘의 {weather_data} 날씨에 {user_mbti} 성향의 사용자가 느낄 감정적 니즈 분석.
    2. {color_tone}에 기반한 색채심리학적 코디 솔루션 (예: 안정을 주는 블루, 자신감을 주는 레드 등).
    3. 추천된 기능성 소재(시어서커/린넨 등)가 오늘 하루의 정서에 미치는 긍정적 영향.
    
    형식: 'LLM NARRATIVE' 컴포넌트에 스트리밍될 수 있도록 감성적이고 정중한 구어체로 300자 내외 작성.
    """
    return prompt.strip()
