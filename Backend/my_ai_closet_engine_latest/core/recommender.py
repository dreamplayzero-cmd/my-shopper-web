...
def get_color_psychology_match(state, color):
    """심리 상태와 색상 매칭 보조 함수"""
    mapping = {
        'CALM': ['블루', '그린', '네이비', '민트', 'blue', 'green', 'navy', 'mint'],
        'ENERGY': ['오렌지', '옐로우', '레드', 'orange', 'yellow', 'red'],
        'CONFIDENCE': ['블랙', '퍼플', '화이트', 'black', 'purple', 'white']
    }
    target_colors = mapping.get(state, [])
    return any(tc in color.lower() for tc in target_colors)

def recommend_with_sqlite(user_id, tpo, weather_tag):
    """SQLite를 이용한 가중치 기반 정밀 추천 엔진 (성별 필터 및 다양성 로직 포함)"""
    import random
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    # 유저 성향 및 성별 가져오기
    cursor.execute("SELECT mbti, psychology_state, gender FROM User WHERE id = ?", (user_id,))
    user_data = cursor.fetchone()
    if not user_data:
        mbti, psych_state, user_gender = "Unknown", "CALM", "남성"
    else:
        mbti, psych_state, user_gender = user_data
    
    # 성별 필터 적용 (Product 테이블의 gender가 '남성', '여성' 또는 NULL인 경우 고려)
    # Reference 스타일 제외하고 '내 옷장'과 '구매 후보'에서 추출
    query = "SELECT * FROM Product WHERE type != 'REFERENCE'"
    params = []
    if user_gender:
        query += " AND (gender = ? OR gender IS NULL OR gender = '공용')"
    ...
    for p_tuple in products:
        p = dict(zip(columns, p_tuple))
        score = 0
        
        # 1. 날씨/소재 일치 (+3점)
        if p.get('weather_tag') == weather_tag:
            score += 3
            
        # 2. 유저 MBTI 성향 매칭 (+2점)
        # mood_tag나 style 필드에 MBTI 키워드가 있는지 확인
        if p.get('mood_tag') and mbti in p.get('mood_tag'):
            score += 2
        elif p.get('style') and mbti in p.get('style'):
            score += 2
            
        # 3. 색채심리 태그 일치 (+2점)
        if get_color_psychology_match(psych_state, p.get('color', '')):
            score += 2
            
        # 4. 다양성 부여 (동일 점수인 경우 순서가 고정되지 않도록 0~0.9 사이의 난수 추가)
        p['score'] = score + random.random()
    ...
    conn.close()
    return scored_products[:30] # 상위 30개 반환
